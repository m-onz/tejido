defmodule TajidoHarmonyTest do
  use ExUnit.Case
  
  test "constrains notes to scale" do
    # C Major scale (C D E F G A B)
    assert Tajido.Harmony.constrain_to_scale("60 61 65 68", :major, "C") == "60 60 65 67"
    
    # C Minor scale (C D Eb F G Ab Bb)
    assert Tajido.Harmony.constrain_to_scale("60 63 65 68", :minor, "C") == "60 63 65 68"
    
    # Test with rests
    assert Tajido.Harmony.constrain_to_scale("- 61 - 66", :major, "C") == "- 60 - 65"
  end
  
  test "constrains notes to chord" do
    # C Major chord (C E G)
    assert Tajido.Harmony.constrain_to_chord("60 61 63 66", :major, "C") == "60 60 64 67"
    
    # C Minor7 chord (C Eb G Bb)
    assert Tajido.Harmony.constrain_to_chord("60 62 65 68", :minor7, "C") == "60 63 63 67"
  end
  ...
  test "harmonic sequence follows chord progression" do
    ...
    # C Major (C E G)
    assert Tajido.Harmony.harmonic_sequence(pattern, chords, position: 0) == "60 64 67 72"
    
    # F Major (F A C)
    assert Tajido.Harmony.harmonic_sequence(pattern, chords, position: 1) == "60 65 65 69"
    
    # G7 (G B D F)
    assert Tajido.Harmony.harmonic_sequence(pattern, chords, position: 2) == "59 62 67 71"
  end
  
  test "constrains to intervals" do
    # Major chord intervals from C (C E G C)
    assert Tajido.Harmony.constrain_to_intervals("60 62 66 69", 60, [0, 4, 7, 12]) == "60 60 67 67"
    
    # Minor chord intervals from C (C Eb G)
    assert Tajido.Harmony.constrain_to_intervals("- 61 - 72", 60, [0, 3, 7, 10]) == "- 60 - 72"
  end
  ...
  test "applies tension to patterns" do
    ...
    assert Tajido.Harmony.tension("60 62 65 67", 60, 0.0) == "60 60 64 67"
    
    # Some tension (balanced)
    tension_result = Tajido.Harmony.tension("60 62 65 67", 60, 0.5)
    assert tension_result != "60 60 64 67"  # Should be different from full resolution
    
    # Full tension - dissonant intervals
    _tension_result = Tajido.Harmony.tension("60 62 65 67", 60, 1.0)
  ...
  test "applies voice leading" do
    ...
    result = Tajido.Harmony.voice_leading(original, :major, "C")
  ...
  test "generates chord voicings" do
    # Close voicing
    close = Tajido.Harmony.chord_voicing("C", :major7, style: :close)
    assert close == "60 64 67 71"  # C E G B
    
    # Spread voicing
    spread = Tajido.Harmony.chord_voicing("C", :major7, style: :spread)
    ...
    # Drop 2 voicing
    drop2 = Tajido.Harmony.chord_voicing("C", :major7, style: :drop2)
